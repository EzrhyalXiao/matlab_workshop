function [real,forecasts,adjust,f,weight,e_new] = nowcast2(Res,newdata,Variable)
index=0;
...
date=datetime(table2array(newdata(:,1)));
newdata=table2array(newdata(:,2:end));
A=Res.H;
H=Res.A;
mu=Res.mu;
Q=Res.R;
...
plot(date,real,'k')
hold on
scatter(date,forecasts(:,index),'x')
...
attention_adjust(3)=1;
attention_adjust=attention_adjust*(size(z,1))/(size(z,1)+attention_adjust(2)-1);
...
for j=2:size(z,2)+1
    ...
    e_new(logical(W(i,:)),ti)=z(logical(W(i,:)),i)-H(logical(W(i,:)),:)*x_po(:,j);
    for k=1:size(Bu,1)
        if ~logical(W(i,k))
    ...
    x_adjust(logical(W(i,:)),i)=(loading'*K)'.*(z(logical(W(i,:)),i)-H(logical(W(i,:)),:)*x_pri-Bu(logical(W(i,:))));
...
function[f,x_adjust,K1,forecasts2]=KF2(z,x0,A,H,Q,R,mu,W,q1,q2,K1,loading)
%x_k=Ax_k-1+w~Q
%W_k*z_k=W_k*Hx_k+W_k*mu+W_k*v~R x向量上到下为滞后q1 q1-1 q1-2...
%注意向量的格式
%W为缺失值标记
...
for j=2:size(z,2)+1
    ...
    %K=(P_new*H(logical(W(i,:)),:)')/((H(logical(W(i,:)),:))*P_new*H(logical(W(i,:)),:)' ...
        %+R(logical(W(i,:)),logical(W(i,:))));
    %%%x_adjust(:,i-1)=K*(z(logical(W(i,:)),i)-H(logical(W(i,:)),:)*x_pri);
    x_po(:,j)=x_pri+K*(z(logical(W(i,:)),i)-H(logical(W(i,:)),:)*x_pri);
    %forecasts2(logical(W(i,:)),i)=(H(logical(W(i,:)),:)*x_pri);
    forecasts2(:,i)=(H*x_pri);
    x_adjust(logical(W(i,:)),i)=(loading'*K)'.*(z(logical(W(i,:)),i)-H(logical(W(i,:)),:)*x_pri);
    P=P_new-K*H(logical(W(i,:)),:)*P_new;
end
%P_new=A*P*A'+Q;
%K=(P_new*H')/(H*P_new*H'+R);
