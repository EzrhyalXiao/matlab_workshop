function[Res]=dfm_arq_attention(y,N,T,q1,q2,attention,lambda)
...
attention_adjust=ones(1,size(y,2)-1);
attention_adjust(index)=lambda;
attention_adjust=attention_adjust*(size(y,2))/(size(y,2)+lambda-1);
...
n=1;
while (judge>1e-4)
    [f,K]=e_step(y,w,A,H,Q,R,mu,q1,q2,attention_adjust);
...
%%用卡尔曼滤波做e-step
function[f,K]=KF2(z,A,H,Q,R,mu,W,q1,q2,attention_adjust)
%x_k=Ax_k-1+w~Q
...
function[f,K]=e_step(y,w,A,H,Q,R,mu,q1,q2,attention_adjust)
[f,K]=KF2(y',H',A',R,diag(Q),mu',w,q1,q2,attention_adjust);
end
